...
const customerAndAge = (obj) => {
  return Object.keys(obj).map(
    (key) => `Customer Name :${key} , Age :${obj[key]}`
  );
};
...
const getInfo = (arr) => {
  ...
  arr.forEach((item) =>
    item.Students.forEach((student) => studentsName.push(student))
  );
...
const getStudents = (arr) => {
  ...
  arr.forEach((student) => {
    courses.forEach((course) => {
      course.Students.find((stu) => stu === student)
        ? newData.push({ Student: student, course: course.course })
        : "";
    });
  });
